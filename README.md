...
- **Worker training & certification** — phone-keyed `Trainee` rows (no `User` row needed) get an SMS magic link to a mobile-first reading + multi-question exam + declaration flow under `/training/take/<token>`. Magic-link tokens are HMAC-SHA256-signed (`{enrolment_id}.{exp}.{sig}`), TTL-bound (default 7 days, configurable per course), and resumable so a worker can read on a break and come back. Course content lives in versioned `TrainingCourseVersion` rows (mirroring the Pipeline pattern — admin edits create v+1; in-flight enrolments stay pinned to their original version). Declaration captures both **typed name and a drawn signature** (vendored SignaturePad, no CSP relaxation; signature stored as PNG blob with IP + UA). On pass, a `TrainingCertification` is issued with a configurable validity window and a WeasyPrint PDF certificate. Re-cert is **dual-driven**: a `flask training-scheduler` long-running CLI handles routine annual cycles; a new `ResponderType.SEND_TRAINING_LINK` plugs into the existing trigger/responder engine for incident-driven enrolment (e.g. "every operator on LINE_A on a HIGH-severity CCP deviation"). Compliance + plant-manager dashboard at `/admin/training/dashboard` shows the per-trainee × per-course matrix. Three new permissions (`training.author`, `training.review`, `training.send`) gate the admin surface; FSA / SALSA / BRC retailer audits can pull the cert PDFs and the chain-hashed audit trail directly.
